# Bellwright Wiki

![GitHub Actions Workflow Status](https://img.shields.io/github/actions/workflow/status/r0ute/bw-wiki/pages.yml)
![GitHub last commit](https://img.shields.io/github/last-commit/r0ute/bw-wiki)

Extracted and generated data from the **Bellwright** game.

## Contents

- `docs/` — Generated Bellwright wiki documentation
- `assets/` — Extracted game assets
- `generator/` — Data scanning, schemas, and Markdown generation code

Generated documentation mirrors the extracted game category structure and includes item properties, icons, quests, NPCs, resources, and other game data.

## Site

[View the Bellwright Wiki](https://r0ute.github.io/bw-wiki/)

The documentation covers Bellwright game data, including items, quests, NPCs, resources, crafting, and gameplay information.